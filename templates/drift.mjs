...
const MODEL = 'CHANGE-ME';                                  // e.g. 'shipgate' → architecture/shipgate/workspace.dsl
const SOURCES = ['server', 'web/src'];                      // e.g. ['src'], ['api', 'app']
export const SHARED = new Set(['server/domain.ts', 'web/src/ui.tsx']);

const DSL = path.join(ROOT, 'architecture', MODEL, 'workspace.dsl');
const IGNORED = (f) => /\.test\.[jt]sx?$/.test(f) || /\.spec\.[jt]sx?$/.test(f) || /\.d\.ts$/.test(f);
